...
public class Session extends RequestGenerator
{
    ...
    private final Map<Type, List<Task>> taskMap = new ConcurrentHashMap<Type, List<Task>>();
    ...
    public void onEvent(Task task, Type type)
    {
        if (!taskMap.containsKey(type))
        {
            ...
            taskMap.put(Type.DEFAULT, tasks);
        }
        else
        {
        ...
        if (!taskMap.get(Type.DEFAULT).contains(task))
        {
            taskMap.get(Type.DEFAULT).add(task);
        }
    }
    ...
    public Channel getChannel(String channelName)
    {
        if (con != null)
        {
            return con.getChannel(channelName);
        }
        return null;
    }
